...
def estimate_fiber_properties(
    ...
    pcy, pcx = (window_radius * 2 + 1) / 2.0, (window_radius * 2 + 1) / 2.0
    ...
    def calc_orientation_tensor(patch):
        ...
        tensor_vals = np.array(
            [[np.mean(Axx), np.mean(Axy)], [np.mean(Axy), np.mean(Ayy)]]
        )
    ...
    for i, (yy, xx) in enumerate(zip(ycords, xcords)):
        ...
        if clear_padded_fiber_skel[yy, xx] != 0:
            ...
            fiber_tpatch = padded_fiber_mask[
                yy - diameter_window_radius : yy + diameter_window_radius + 1,
...
def estimate_fourier_orientation(data, grid_shape=(2, 2), sigma=2.0, zoom=1.0, order=3):
    ...
    for i in xrange(grid_shape[0]):
        for j in xrange(grid_shape[1]):
            y0, x0 = (
                block_shape[0] * j + block_shape[0] / 2,
...
def _filter_coords(data, ws, th_nval=0.8):
    ...
    def check_point(data, pt, ws2, th):
        ...
        if np.count_nonzero(vol) < ws2 * 2.0 * th:
            return False
    ...
    filtered_coords = [
        pt
        for pt in np.transpose(np.nonzero(data))
        if check_point(data, pt, ws2, th_nval)
    ]
...
def orientation_3d_tensor_vigra(data, sigma=0.1):
    ...
    tensor_vals = np.array(
        [
            [np.mean(Azz), np.mean(Ayz), np.mean(Axz)],
    ...
    azth = np.arctan(mv[1] / mv[2]) if mv[2] else np.pi / 2.0

    return (lat, azth)


def estimate_tensor(
    name, skel, data, window_size, output_dir, sigma=0.025, make_output=True
):
...
def estimate_diameter_gpu(
    ...
    out = {"diameter": radius_arr * 2.0, "time": dm_time}
    return out


def estimate_diameter_single_run(
    name,
    output_dir,
    data,
    skel,
    ...
    output_props["time"] = 0.0
    output_props["sample_name"] = name
    output_props["type"] = "diameter_gpu"
    output_props["n_processes"] = 1

    result = estimate_diameter_gpu(
        skel, data, lat_data, azth_data, n_scan_angles, do_reshape=False
    )
...
def estimate_diameter_batches(
    ...
    for batch_idxs in batches_idxs:
        ...
        if batch_idxs[0] == 0:
            ...
            arr1, arr2 = (
                np.arange(batch_idxs[0] - border_gap, batch_idxs[0]),
        ...
        gaped_out_dict = estimate_diameter_gpu(
            batched_skel,
            batched_data,
            batched_lat_data,
            batched_azth_data,
        ...
        for data_name in out_arr_names:
            ...
            if batch_idxs[0] == 0:
                ...
                output[data_name][batch_idxs] = gaped_arr[
